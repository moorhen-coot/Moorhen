...
describe('MoorhenNumberInput', () => {
    ...
    test('waitReturn does not call setValue on keystroke (only on Enter)', async () => {
        ...
        render(
            <Provider store={MoorhenReduxStore}>
                <MoorhenNumberInput value={0} setValue={setValue} waitReturn={true} />
            </Provider>
        )
        const input = screen.getByRole('textbox')
        await user.clear(input)
    ...
    test('clamps value to minMax range on blur', async () => {
        ...
        render(
            <Provider store={MoorhenReduxStore}>
                <MoorhenNumberInput value={50} setValue={setValue} minMax={[0, 100]} waitReturn={true} />
            </Provider>
        )
        const input = screen.getByRole('textbox')
        await user.clear(input)
    ...
    test('rejects negative input when allowNegativeValues is false', async () => {
        ...
        render(
            <Provider store={MoorhenReduxStore}>
                <MoorhenNumberInput value={10} setValue={setValue} allowNegativeValues={false} waitReturn={true} />
            </Provider>
        )
        const input = screen.getByRole('textbox')
        await user.clear(input)
    ...
    test('renders +/- arrow buttons when type is number', () => {
        render(
            <Provider store={MoorhenReduxStore}>
                <MoorhenNumberInput value={50} setValue={() => {}} type="number" />
            </Provider>
        )
        const buttons = screen.getAllByRole('button')
    ...
    test('renders label on top when labelPosition is top', () => {
        const { container } = render(
            <Provider store={MoorhenReduxStore}>
                <MoorhenNumberInput value={0} label="Top Label" labelPosition="top" />
            </Provider>
        )
