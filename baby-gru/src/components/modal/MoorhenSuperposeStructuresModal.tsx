...
const lsqkbResidueRangesReducer = (
    ...
    change: { action: string; item?: moorhen.lskqbResidueRangeMatch }
) => {
                ...
                <>
                    ...
                    {lsqkbResidueRanges.length > 0 ? (
                        lsqkbResidueRanges.map(item => {
                            return (
                                <LskqbResidueRangeMatchCard
                                    key={JSON.stringify(item)}
                                    refMolNo={selectedRefModel}
                                    movMolNo={selectedMovModel}
                                    item={item}
                                    removeItem={() => setLsqkbResidueRanges({ action: "remove", item: item })}
                                />
                            );
                        })
                    ) : (
