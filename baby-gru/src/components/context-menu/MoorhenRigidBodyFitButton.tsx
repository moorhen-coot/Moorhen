...
export const MoorhenRigidBodyFitButton = (props: ContextButtonProps) => {
    ...
    const rigidBodyFitFormatArgs = (
        ...
        switch (selectedMode) {
            case "RESIDUE":
                commandArgs = [molecule.molNo, `//${chosenAtom.chain_id}/${chosenAtom.res_no}`, activeMapMolNo];
                break;
            case "CHAIN":
                commandArgs = [molecule.molNo, `//${chosenAtom.chain_id}/*`, activeMapMolNo];
                break;
            case "MOLECULE":
                commandArgs = [molecule.molNo, `//*/*`, activeMapMolNo];
                break;
            case "CUSTOM":
                commandArgs = [molecule.molNo, customCid.current, activeMapMolNo];
                break;
            default:
    ...
    const getCootCommandInput = useCallback(
        (
            selectedMolecule: moorhen.Molecule,
            chosenAtom: moorhen.ResidueSpec,
            selectedMode: string,
            randomJiggleModeSelectRef?: React.RefObject<HTMLInputElement>
        ) => {
