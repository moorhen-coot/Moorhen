...
export const MoorhenModalsContainer = (props: { extraDraggableModals: ExtraDraggableModals }) => {
    const showCreateAcedrgLinkModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.ACEDRG)
    );
    const showQuerySequenceModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.SEQ_QUERY)
    );
    const showScriptingModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.SCRIPTING)
    );
    const showControlsModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.SHOW_CONTROLS)
    );
    const showFitLigandModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.FIT_LIGAND)
    );
    const showRamaPlotModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.RAMA_PLOT)
    );
    const showDiffMapPeaksModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.DIFF_MAP_PEAKS)
    );
    const showValidationPlotModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.VALIDATION_PLOT)
    );
    const showMmrrccModal = useSelector((state: moorhen.State) => state.modals.activeModals.includes(modalKeys.MMRRCC));
    const showWaterValidationModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.WATER_VALIDATION)
    );
    ...
    const showMoorhenMrBumpModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.MRBUMP)
    );
    const showMoorhenMrParseModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.MRPARSE)
    );
    const showPepFlipsValidationModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.PEPTIDE_FLIPS)
    );
    const showUnmodelledBlobsModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.UNMODELLED_BLOBS)
    );
    ...
    const showSliceNDiceModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.SLICE_N_DICE)
    );
    const showSuperposeModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.SUPERPOSE_MODELS)
    );
    const showSceneSettingsModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.SCENE_SETTINGS)
    );
    ...
    const showJsonValidationModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.JSON_VALIDATION)
    );
    const showColorMapByMapModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.COLOR_MAP_BY_MAP)
    );
    const showVectorsModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.VECTORS)
    );
    const showOverlays2DModal = useSelector((state: moorhen.State) =>
        state.modals.activeModals.includes(modalKeys.OVERLAYS2D)
    );
