    ...
    doMouseMove(event,self) {
        ...
        if(this.state.plotInfo){
            ...
            if(hit>-1){
                ...
                this.props.setHoveredAtom(`//${this.state.chainId}/${this.state.plotInfo[hit].seqNum}${this.state.plotInfo[hit].insCode ? '.' + this.state.plotInfo[hit].insCode : ''}(${this.state.plotInfo[hit].restype})/`)
            };
        }
    }
