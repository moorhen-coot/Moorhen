...
std::vector<TableEntry> validate(const std::string &file, const std::string &name)
{
  ...
  std::string ifilename = "/privateer_torsions_z_score_database.json";
  std::ifstream ifile;
  std::string ofilename = "/privateer_torsions_z_score_database_fix.json";
   std::ofstream ofile;
  ...
  while(std::getline(ifile, text)) {
    std::string fixed_text = std::regex_replace(text, std::regex("/\\/"), "");
    ofile << fixed_text;
