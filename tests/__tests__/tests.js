...
describe('Testing molecules_container_js', () => {
    ...
    test('Test Auto-fit rotamer', () => {
        ...
        const CD1atom = res.GetAtom(4);
        expect(CD1atom.x).toBeCloseTo(51.473,3)
        expect(CD1atom.y).toBeCloseTo(10.418,3)
        expect(CD1atom.z).toBeCloseTo(6.145,3)
        expect(CD1atom.GetSeqNum()).toBe(89)
        expect(CD1atom.GetResidueNo()).toBe(88)
        expect(CD1atom.tempFactor).toBeCloseTo(36.17,2)
        expect(CD1atom.occupancy).toBeCloseTo(1.00,2)
        expect(CD1atom.charge).toBeCloseTo(0.00,2)
        expect(CD1atom.GetNBonds()).toBe(0)
        const result = molecules_container.auto_fit_rotamer(imol, "A", 89, "", "", imol_map)
        expect(result).toBe(1)
        expect(res.nAtoms).toBe(11)
        expect(res.GetResName()).toBe("PHE")
        expect(res.GetChainID()).toBe("A")
        expect(res.GetSeqNum()).toBe(89)
        expect(res.GetResidueNo()).toBe(88)
        expect(res.GetInsCode()).toBe("")
        const Natom = res.GetAtom(0);
        expect(Natom.x).toBeCloseTo(52.477,3)
        expect(Natom.y).toBeCloseTo(13.485,3)
        expect(Natom.z).toBeCloseTo(8.124,3)
        expect(Natom.GetSeqNum()).toBe(89)
        expect(Natom.GetResidueNo()).toBe(88)
        expect(Natom.tempFactor).toBeCloseTo(16.15,2)
        expect(Natom.occupancy).toBeCloseTo(1.00,2)
        expect(Natom.charge).toBeCloseTo(0.00,2)
        expect(Natom.GetNBonds()).toBe(0)
        const CD1RotAtom = res.GetAtom(4);
        expect(CD1RotAtom.x).toBeCloseTo(53.524,3)
        expect(CD1RotAtom.y).toBeCloseTo(10.602,3)
        expect(CD1RotAtom.z).toBeCloseTo(10.393,3)
        expect(CD1RotAtom.GetSeqNum()).toBe(89)
        expect(CD1RotAtom.GetResidueNo()).toBe(88)
        expect(CD1RotAtom.tempFactor).toBeCloseTo(36.17,2)
        expect(CD1RotAtom.occupancy).toBeCloseTo(1.00,2)
        expect(CD1RotAtom.charge).toBeCloseTo(0.00,2)
        expect(CD1RotAtom.GetNBonds()).toBe(0)
    })
