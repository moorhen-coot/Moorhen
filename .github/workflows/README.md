...
* nightly-tests - As the name implies: Runs every night. Builds the entirety of Moorhen with all dependencies and runs 'npm test' (probably same as 'npm run test'). Very wasteful: Needs improved dependency caching. How does 'npm test' differ from 'npm test-react'? Deleted because now 'run-tests' does all that in an efficient way.
